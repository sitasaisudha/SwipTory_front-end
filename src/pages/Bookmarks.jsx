...
const Bookmarks = () => {
        ...
        const slidesData = await Promise.all(
          bookList.map(async (slideId) => {
            const response = await axios.get(
              `${baseUrl}/api/slideinfo/?id=${slideId}`
            );
  ...
  const handlePrevSlide = () => {
    setCurrentSlideIndex((prevIndex) =>
      prevIndex > 0 ? prevIndex - 1 : bookSlides.length - 1
    );
  ...
  const handleNextSlide = () => {
    setCurrentSlideIndex((prevIndex) =>
      prevIndex < bookSlides.length - 1 ? prevIndex + 1 : 0
    );
    setSelectedSlide(bookSlides[currentSlideIndex]);
    setShare(false);
  };
      ...
      >
        <div className="book-carousel">
          <div className="book-carousel-slide">
            <img
              src={selectedSlide && selectedSlide.image_url}
              alt={selectedSlide && selectedSlide.heading}
            />
          ...
          <div className="carousel-btns"></div>
        </div>
      </Modal>
    </div>
  );
